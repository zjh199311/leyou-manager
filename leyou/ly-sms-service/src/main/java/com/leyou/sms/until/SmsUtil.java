...
public class SmsUtil {
    ...
    public CommonResponse sendSms(String signName, String templateCode, String phoneNumbers, Map<String, Object> templateParam) {
        try {
            String key = SMS_PREFIX + phoneNumbers;
            ...
            if (StringUtils.isNotBlank(lastTime)) {
                ...
                if (System.currentTimeMillis() - time < SMS_MIN_INTERVAL_IN_MILLIS) {
                    ...
                    log.info("【短信服务】短信发送频率过高，被拦截，手机号：{}", phoneNumbers);
                    return null;
                }
            }
            ...
            request.putQueryParameter("电话", phoneNumbers);
            ...
            if ("ok".equals(map.get("Code"))) {
                ...
                log.error("【消息服务】消息发送未成功，手机号码：{}，原因：{}", phoneNumbers, map.get("Message"));
                return null;
            }
        } catch (Exception e) {
            log.error("【短信服务】发送信息失败，手机号码：{}", phoneNumbers, e);
            return null;
        }
    }
}
